...
test("GET /auctions/:id/reports/export returns CSV for AdminL2", async () => {
  const handler = createApiHandler(createBaseDeps());
  const req = createMockRequest(
    "GET",
    "/auctions/auction-1/reports/export",
    "Bearer token",
    {},
    {format: "csv"}
  );
  ...
  assert.match(res.body, /^bidderId,bidderNumber,displayName,subtotal,total,paid/);
  assert.match(res.body, /bidder-2,3,Bidder Two,250,250,true/);
  assert.match(res.body, /bidder-1,5,Bidder One,100,100,false/);
});

test("GET /auctions/:id/reports/export returns 403 for bidder role", async () => {
  const handler = createApiHandler(createBaseDeps({
    getUserById: async () => ({
      id: "bidder-1",
      role: "Bidder",
